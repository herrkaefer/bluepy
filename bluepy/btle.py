...
class UUID(object):
    def __init__(self, val, commonName=None):
        ...
        if len(self.binVal) != 16:
            raise ValueError("UUID must be 16 bytes, got '%s' (len=%d)"
                             % (val, len(self.binVal)))
...
class Service(object):
    ...
    def getCharacteristics(self, forUUID=None):
        if not self.chars:  # Unset, or empty
            self.chars = [] \
                ...
                else self.peripheral.getCharacteristics(self.hndStart,
                                                        self.hndEnd)
    ...
    def getDescriptors(self, forUUID=None):
        if not self.descs:
            ...
            all_descs = self.peripheral.getDescriptors(self.hndStart+1,
                                                       self.hndEnd)
    ...
    def __str__(self):
        return ("Service <uuid=%s handleStart=%s handleEnd=%s>"
                % (self.uuid.getCommonName(), self.hndStart, self.hndEnd))
...
class Characteristic(object):
    ...
    def write(self, val, withResponse=False):
        return self.peripheral.writeCharacteristic(
            self.valHandle, val, withResponse)
    ...
    def getDescriptors(self, forUUID=None, hndEnd=0xFFFF):
        if not self.descs:
            ...
            self.descs = []
            for desc in self.peripheral.getDescriptors(self.valHandle+1,
                                                       hndEnd):
    ...
    def _mgmtCmd(self, cmd):
        ...
        if rsp['code'][0] != 'success':
            ...
            raise BTLEManagementError(
                "Failed to execute management command '%s'" % (cmd), rsp)
    ...
    def _waitResp(self, wantType, timeout=None):
        while True:
            ...
            if respType in wantType:
                ...
                if ('state' in resp
                   and len(resp['state']) > 0
                   and resp['state'][0] == 'disc'):
                    self._stopHelper()
...
class Peripheral(BluepyHelper):
    def __init__(self, deviceAddr=None, addrType=ADDR_TYPE_PUBLIC, iface=None):
        ...
        if isinstance(deviceAddr, ScanEntry):
            self._connect(
                deviceAddr.addr, deviceAddr.addrType, deviceAddr.iface)
    ...
    def _connect(self, addr, addrType=ADDR_TYPE_PUBLIC, iface=None):
        ...
        if addrType not in (ADDR_TYPE_PUBLIC, ADDR_TYPE_RANDOM):
            raise ValueError("Expected address type public or random, got {}"
                             .format(addrType))
    ...
    def discoverServices(self):
        ...
        for i in range(nSvcs):
            self._serviceMap[UUID(uuids[i])] = Service(self, uuids[i],
                                                       starts[i], ends[i])
    ...
    def getServiceByUUID(self, uuidVal):
        ...
        if 'hstart' not in rsp:
            raise BTLEGattError(
                "Service %s not found" % (uuid.getCommonName()), rsp)
    ...
    def _setRemoteOOB(self, address, address_type, oob_data, iface=None):
        ...
        if oob_data['C_192'] is not None and oob_data['R_192'] is not None:
            cmd += (" C_192 " + oob_data['C_192']
                    + " R_192 " + oob_data['R_192'])
        if oob_data['C_256'] is not None and oob_data['R_256'] is not None:
            cmd += (" C_256 " + oob_data['C_256']
                    + " R_256 " + oob_data['R_256'])
        if iface is not None:
            cmd += " hci"+str(iface)
        self._writeCmd(cmd)
    ...
    def setRemoteOOB(self, address, address_type, oob_data, iface=None):
        ...
        if isinstance(address, ScanEntry):
            return self._setOOB(
                address.addr, address.addrType, oob_data, address.iface)
    ...
    def getLocalOOB(self, iface=None):
        ...
        if resp is not None:
            ...
            if (struct.unpack_from('<B', data, 0)[0] != 8
    ...
    def getValue(self, sdid):
        ...
        if sdid in [ScanEntry.SHORT_LOCAL_NAME, ScanEntry.COMPLETE_LOCAL_NAME]:
            try:
                ...
                # references to 'local name' (e.g. vol 3 E, 6.23) suggest it's
                # UTF-8 but in practice devices sometimes have garbage here.
                # See #259, #275, #292.
                return val.decode('utf-8')
            except UnicodeDecodeError:
                bbval = bytearray(val)
                return ''.join([(chr(x) if (x >= 32 and x <= 127) else '?')
                               for x in bbval])
        elif sdid in [ScanEntry.INCOMPLETE_16B_SERVICES,
                      ScanEntry.COMPLETE_16B_SERVICES]:
            return self._decodeUUIDlist(val, 2)
        elif sdid in [ScanEntry.INCOMPLETE_32B_SERVICES,
                      ScanEntry.COMPLETE_32B_SERVICES]:
            return self._decodeUUIDlist(val, 4)
        elif sdid in [ScanEntry.INCOMPLETE_128B_SERVICES,
    ...
    try:
        for svc in conn.services:
            ...
            for ch in svc.getCharacteristics():
                print("    {}, hnd={}, supports {}"
                      .format(ch, hex(ch.handle), ch.propertiesToString()))
